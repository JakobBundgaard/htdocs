<?php
define('_PASSWORD_MIN_LEN', 4);
define('_PASSWORD_MAX_LEN', 10);
...
//     $error_message = 'item_price must be a whole number or have 2 decimals';
...
function _validate_user_name(){
    $error_message = "uid min " . _USER_NAME_MIN_LEN . " max " . _USER_NAME_MAX_LEN. " characters";
    if( ! isset($_POST['uid']) ){
      _respond($error_message, 400);
    }
    $_POST['uid'] = trim( $_POST['uid']);
    if(strlen( $_POST['uid']) < _USER_NAME_MIN_LEN) {
      _respond($error_message, 400);
    }
    if(strlen( $_POST['uid']) > _USER_NAME_MAX_LEN) {
      _respond($error_message, 400);
    }
    return $_POST['uid'];
  }
